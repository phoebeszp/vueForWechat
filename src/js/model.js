  ...
  function _parse_object(data, model, param) {
    ...
    switch (model.type) {
      case TYPE.OBJECT:
        ...
        let columns = 0;
        if (param.isParse) {
          let keys = Utils.mergeArray(Object.keys(model.value),data?Object.keys(data):[]);
          for (let i of keys) {
            if (model.value.hasOwnProperty(i)) {
              data = data || {};
              param.parentData = out_data;
              let _out = _parse_object(data[i], model.value[i], param);
          ...
          for (let i of Object.keys(data)) {
            if (model.value.hasOwnProperty(i)) {
              param.parentData = out_data;
              let d = _parse_object(data[i], model.value[i], param);
      ...
      case TYPE.ARRAY:
        out_data = [];
        for (let n of data) {
          let r = _parse_object(n, model.value, param);
      ...
      case TYPE.DATE:
        if(Utils.isString(data)&&data==""){
          ...
          if (param.isParse) {
            out_data = manba(data).format(model.format||"");
          } else {
            out_data = manba(data).toISOString();
          }
        }
        break;
      case TYPE.BOOLEAN:
